...
Prod-only repo — **no `staging` branch** (normally). Ship via `release.sh hotfix`.

**Staging-targeted tasks (review-buffer waves):** when a brief mandates a staging review gate + "do NOT promote to prod" (e.g. the cross-service "one filter vocabulary" program), there is no staging branch to PR into. Reconcile by **creating `staging` off `main`** (`git push origin origin/main:refs/heads/staging`) and PR'ing the feature into it — `main` IS prod here, so this keeps prod untouched while giving the buffer. The orchestrator promotes staging→main later. Done this way for #30/#31 (apify audiences, 2026-06-25).
...
- `src/routes/verify.ts` — `POST /verify` (org-scoped, same auth tier as /search): verify deliverability for a batch of arbitrary email addresses (1..100). Returns one `{email, status}` per input, `status` ∈ `valid|invalid|risky|catch_all|unknown`. Backed by the `bounceverify~bounceverify-email-verifier` Apify actor (real SMTP + catch-all on its OWN backend — benched 5.6× cheaper AND more accurate than ryanclinton, which does no real SMTP on Apify's port-25-blocked infra). Cost declared (provision→authorize→actualize, fail-loud); no DB persistence (stateless verification, run row is the audit trail). Built for outlets-service's pre-outreach "send only if valid" gate — verifies generic role inboxes (pr@, editor@) that /resolve can't (no person name).
...
- The Apify token is a PLATFORM key. **Lead actors have TWO billable events: a per-RUN `actor-start` fee AND a per-lead fee — declare BOTH** (provision→authorize→execute→actualize, fail-loud). Active cost names: `apify-pipelinelabs-lead` + `apify-pipelinelabs-actor-start` (search/resolve), and `apify-bounceverify-email` (/verify — single per-email event, no actor-start). All must exist in costs-service or provision 422s before spend.
- **`/verify` bills per-EMAIL verified, from what we OBSERVE.** bounceverify has ONE event (`email-verified` $0.00089, no actor-start) and charges only for DECISIVE results (`unknown` rows are free). Worst-case provision = `emails.length`; actualize = `billableCount` (returned rows whose `status` is not `unknown`). The bounceverify `status` enum (`valid|invalid|risky|unknown`) + `is_catch_all` + `is_spamtrap` fold onto our 5-literal enum in `mapVerifyStatus` (invalid→invalid; is_catch_all true→catch_all, before valid; is_spamtrap→risky; unrecognized→unknown). Never `z.string().email()` on the input — syntactically-invalid addresses are valid input (they return `invalid`).
...
- clearpath `clearpath~email-finder-api` — **DISABLED.** Events: `actor_start` $0.005/run + `email_pattern_tested` **$0.008 per pattern tested (hit OR miss)** — bills per attempt, not per email found, so it structurally under-bills the per-delivered-lead model. `people:[{firstName,surname,domain}]`, `mode:"optimized"`; accept only `isSafeToSend && !isCatchAll`. NOTE: clearpath is a FINDER (input = name+domain → guess+verify), NOT a verifier of an existing address — that's why `/verify` uses bulk-email-verifier below.
- bounceverify `bounceverify~bounceverify-email-verifier` — **ACTIVE (POST /verify only).** Single event: `email-verified` $0.00089/email (no actor-start; charges only DECISIVE results, `unknown` is free). Charged 2× margin (cost row `apify-bounceverify-email` base 0.089¢ → 0.178¢). Input `emails:string[]`; output one row per email with `status` (`valid|invalid|risky|unknown`) + `is_catch_all` + `is_spamtrap` + `smtp_valid` + `score`. Real SMTP + catch-all on bounceverify's OWN backend (port-25-capable) — verifies an EXISTING address (no name needed). **Chosen over `ryanclinton~bulk-email-verifier` (the original pick): benched 2026-06-23, ryanclinton does NO real SMTP on Apify infra (port-25 blocked → MX-guessing; marked a nonexistent address "valid"), bounceverify does real SMTP + catch-all at 5.6× lower price.** Mapped to our 5-literal enum in `mapVerifyStatus`.
