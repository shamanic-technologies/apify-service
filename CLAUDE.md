...
- `src/routes/verify.ts` — `POST /verify` (org-scoped, same auth tier as /search): verify deliverability for a batch of arbitrary email addresses (1..100). Returns one `{email, status}` per input, `status` ∈ `valid|invalid|risky|catch_all|unknown`. Backed by the `ryanclinton~bulk-email-verifier` Apify actor (SMTP/MX/catch-all). Cost declared (provision→authorize→actualize, fail-loud); no DB persistence (stateless verification, run row is the audit trail). Built for outlets-service's pre-outreach "send only if valid" gate — verifies generic role inboxes (pr@, editor@) that /resolve can't (no person name).
...
- The Apify token is a PLATFORM key. **Cost has TWO billable events per actor: a per-RUN `actor-start` fee AND a per-lead fee — declare BOTH** (provision→authorize→execute→actualize, fail-loud). Active cost names: `apify-pipelinelabs-lead` + `apify-pipelinelabs-actor-start` (search/resolve), and `apify-bulk-email-verifier-email` + `apify-bulk-email-verifier-actor-start` (/verify). All must exist in costs-service or provision 422s before spend.
- **`/verify` bills like the rest: per-RUN actor-start + per-EMAIL verified, from what we OBSERVE.** Worst-case provision = `emails.length` verify events + 1 actor-start; actualize = count of verification rows the actor RETURNED (`verifiedCount`) + 1 run. The bulk-email-verifier `status` enum (`valid|invalid|risky|disposable|unknown`) + `checks.catchAll` are folded onto our 5-literal enum in `mapVerifyStatus` (invalid→invalid; catchAll true→catch_all, before valid; disposable→risky; unrecognized→unknown). Never `z.string().email()` on the input — syntactically-invalid addresses are valid input (they return `invalid`).
...
- bulk-email-verifier `ryanclinton~bulk-email-verifier` — **ACTIVE (POST /verify only).** Events: `apify-actor-start` $0.00005/run + `email-verified` $0.005/email. Charged 2× margin (cost rows: `apify-bulk-email-verifier-actor-start` base 0.005¢, `apify-bulk-email-verifier-email` base 0.5¢). Input `emails:string[]` (`mode:"raw"`, `verificationLevel:"deep"`); output one row per email with `status` (`valid|invalid|risky|disposable|unknown`) + `checks.catchAll`. SMTP/MX/catch-all/disposable detection — verifies an EXISTING address (no name needed), unlike clearpath. Mapped to our 5-literal enum in `mapVerifyStatus`.
